...
export function ZigZagGrowthStack({
    eyebrow,
    title,
    description,
    cards,
}: ZigZagGrowthStackProps) {
                    ...
                    {cards.map((card, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={
                                    hasDetails
                                        ? `flex flex-col gap-4 lg:items-stretch ${alignRight ? "lg:flex-row-reverse" : "lg:flex-row"
                                        }`
                            ...
                            >
                                ...
                                {hasDetails && (
                                    <aside
                                        ...
                                        className={`overflow-hidden rounded-[8px] border border-[#DFE3EA] bg-white/88 shadow-[0_18px_48px_-34px_rgba(29,39,66,0.32)] backdrop-blur-sm transition-all duration-300 ease-out lg:pointer-events-none ${isRevealOpen
                                            ? `opacity-100 lg:max-w-[430px] ${alignRight ? "lg:translate-x-0" : "lg:translate-x-0"
                                            }`
                                            : `max-h-0 opacity-0 lg:max-h-none lg:max-w-0 ${alignRight ? "lg:translate-x-5" : "lg:-translate-x-5"
                                            }`
                                            }`}
                                    >
                                        <div className="min-w-0 p-6 lg:w-[430px]">
                                            ...
                                            {card.detailCopy && (
                                                <p className="mt-4 text-sm leading-7 text-[#4F5B76]">
                                                    {card.detailCopy}
                                                </p>
                                            )}
